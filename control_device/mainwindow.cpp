...
Simulation_Moudle* moudel_group::find_with_id(int id)
{
...
algorithm::algorithm(QWidget *parent):QDialog(parent)
{
    ...
    vbox->addWidget(main_connect);
    connect(main_connect,&QAbstractButton::pressed,this,&algorithm::clicked_button);
...
void algorithm::init_ui_tag()
{
    ...
    if(this->tag_group->group.size()!=0)
    {
        ...
        for(int i=0;i<this->tag_group->group.size();i++)
        {
            ...
            {
                box->addItems(this->sys_flag->tag_group_temp);
                int index=this->sys_flag->tag_group_temp.indexOf(this->tag_group->group[i]->tag_bind->input_tag);
                stop_flag=true;
                box->setCurrentIndex(index);
                stop_flag=false;
                if(index!=-1)
                {
                    this->sys_flag->input_bind.insert(this->tag_group->group[i]->tag_bind->input_tag,this->tag_group->group[i]);
                }
            ...
            {
                box->addItems(this->sys_flag->tag_group_temp);
                int index=this->sys_flag->tag_group_temp.indexOf(this->tag_group->group[i]->tag_bind->output_tag);
                stop_flag=true;
                box->setCurrentIndex(index);
                stop_flag=false;
                if(index!=-1)
                {
                    this->sys_flag->output_bind.insert(this->tag_group->group[i]->tag_bind->output_tag,this->tag_group->group[i]);
                }
                box_to_moudle.insert(box,this->tag_group->group[i]);
...
void algorithm::combbox_change(QString str)
{
    ...
    if(this->box_to_flag.contains(box)==true)
    {
        ...
        {
            if(this->sys_flag->tag_group.indexOf(str)!=-1)
            {
                ...
                if(this->sys_flag->input_bind.contains(this->box_to_moudle[box]->tag_bind->input_tag)==true)
                    this->sys_flag->input_bind.remove(this->box_to_moudle[box]->tag_bind->input_tag);
                this->sys_flag->input_bind.insert(str,this->box_to_moudle[box]);
                this->box_to_moudle[box]->tag_bind->input_tag=str;
                this->remove_tag(str);
            }
            else
            {
                int index=this->sys_flag->tag_group_temp.indexOf(this->box_to_moudle[box]->tag_bind->input_tag);
                stop_flag=true;
                box->setCurrentIndex(index);
                stop_flag=false;
                this->text_edit->setText(str+" already exists");
            }
            //qDebug()<<this->sys_flag->input_bind.keys();
        }
        else
        {
            if(this->sys_flag->tag_group.indexOf(str)!=-1)
            {
                ...
                if(this->sys_flag->output_bind.contains(this->box_to_moudle[box]->tag_bind->output_tag)==true)
                    this->sys_flag->output_bind.remove(this->box_to_moudle[box]->tag_bind->output_tag);
                this->sys_flag->output_bind.insert(str,this->box_to_moudle[box]);
                this->box_to_moudle[box]->tag_bind->output_tag=str;
                this->remove_tag(str);
            }
            else
            {
                int index=this->sys_flag->tag_group_temp.indexOf(this->box_to_moudle[box]->tag_bind->output_tag);
                stop_flag=true;
                box->setCurrentIndex(index);
                stop_flag=false;
                this->text_edit->setText(str+" already exists");
            }
            //qDebug()<<this->sys_flag->output_bind.keys();
        }
    }
}
...
bool algorithm::detect_connect()
{
    ...
    for(int i=0;i<this->socket_group->group.size();i++)
    {
        ...
        {
            this->text_edit->setText(this->socket_group->group[i]->name+" output connect failed");
            return false;
        }
    }
    if(this->main_connect->text()=="connect")
    {
        this->text_edit->setText("main connect failed");
        return false;
    }
    return true;
}
void algorithm::clicked_button()
{
    ...
    else//main_connect
    {
        if(button->text()=="connect")
        {
            QString ip=socket_main->address[0]->text()+"."+socket_main->address[1]->text()+"."+socket_main->address[2]->text()+"."+socket_main->address[3]->text();
            QHostAddress host;
            host.setAddress(ip);
            this->sys_flag->socket_main->connectToHost(ip,quint16(socket_main->port->text().toInt()));
        }
        else if(button->text()=="disconnect")
        {
            this->sys_flag->socket_main->disconnectFromHost();
        }
    }
}
QPushButton* algorithm::find_with_socket(QTcpSocket* socket)
{
    ...
    {
        if((this->button_to_ui[button_group[i]]->way==1&&this->button_to_ui[button_group[i]]->moudle->bind_interface->output_socket==socket)||(this->button_to_ui[button_group[i]]->way==0&&this->button_to_ui[button_group[i]]->moudle->bind_interface->input_socket==socket))
            return button_group[i];
    }
    return nullptr;
}
algorithm_file::algorithm_file(QWidget *parent):QWidget(parent)
{
    readdata();
    ...
    relation_init();
    detect_loop();//闭环检测
    //qDebug()<<this->simulation_group->group[0]->tag_bind->input_flag<<this->simulation_group->group[0]->tag_bind->input_tag;
    run_();
}
...
void algorithm_file::readdata()
{
    ...
    if(!filename.isNull())
    {
        ...
        double d_temp;
        QString s_temp;
        moudle_count_=getstr(str).toInt();
        for(int i=0;i<moudle_count_;i++)
        {
        ...
        for(int i=0;i<this->simulation_group->group.size();i++)
        {
            ...
            for(int j=0;j<moudle->output.size();j++)
            {
                for(int m=0;m<moudle->output[j]->next.size();m++)
                    moudle->output[j]->next[m].moudle=this->simulation_group->find_with_id(moudle->output[j]->next[m].id);
            }
            if(moudle->index==39)
            {
...
void algorithm_file::remove_tag(QString tag)
{
    if(this->sys_flag->tag_group.indexOf(tag)!=-1)
    {
        int index=this->sys_flag->tag_group.indexOf(tag);
...
void algorithm_file::save_algorithm()
{
    QString savefile=this->url;
    QString str="";
    if(!savefile.isNull())
    {
        ...
        str=str+"!"+QString::number(this->simulation_group->group.size());
        for(int i=0;i<this->simulation_group->group.size();i++)
        {
            Simulation_Moudle* moudle=this->simulation_group->group[i];
...
void algorithm_file::keyPressEvent(QKeyEvent *event)
{
...
void algorithm_file::init()
{
    ...
    for(int i=0;i<this->simulation_group->group.size();i++)
    {
        ...
        {
            if(simulation_group->group[i]->bind_interface->input_flag==true)
            {
                simulation_group->group[i]->bind_interface->input_socket=new QTcpSocket();
                connect(simulation_group->group[i]->bind_interface->input_socket,&QTcpSocket::connected,this,&algorithm_file::socket_connect);
                connect(simulation_group->group[i]->bind_interface->input_socket,&QTcpSocket::disconnected,this,&algorithm_file::socket_disconnect);
                connect(simulation_group->group[i]->bind_interface->input_socket,&QIODevice::readyRead,this,&algorithm_file::socket_receive);
            }
            if(simulation_group->group[i]->bind_interface->output_flag==true)
            {
                simulation_group->group[i]->bind_interface->output_socket=new QTcpSocket();
                connect(simulation_group->group[i]->bind_interface->output_socket,&QTcpSocket::connected,this,&algorithm_file::socket_connect);
                connect(simulation_group->group[i]->bind_interface->output_socket,&QTcpSocket::disconnected,this,&algorithm_file::socket_disconnect);
                connect(simulation_group->group[i]->bind_interface->output_socket,&QIODevice::readyRead,this,&algorithm_file::socket_receive);
            }
        }
    }
    this->sys_flag->socket_main=new QTcpSocket();
    connect(this->sys_flag->socket_main,&QTcpSocket::connected,this,&algorithm_file::socket_connect);
    connect(this->sys_flag->socket_main,&QTcpSocket::disconnected,this,&algorithm_file::socket_disconnect);
    connect(this->sys_flag->socket_main,&QIODevice::readyRead,this,&algorithm_file::socket_receive);
}
...
void algorithm_file::init_finished_flag()
{
...
void algorithm_file::socket_connect()
{
    ...
    if(index!=-1)
    {
        if(start_run==false)
        {
            QPushButton* button=this->alg->find_with_socket(socket);
            button->setText("disconnect");
            ui_socket* ui=this->alg->button_to_ui[button];
            if(ui->way==0)
            {
                for(int i=0;i<4;i++)
    ...
    else
    {
        ...
        if(start_run==false)
        {
            this->alg->main_connect->setText("disconnect");
            this->sys_flag->ip_main="";
            for(int i=0;i<4;i++)
            {
                this->sys_flag->ip_main+=this->alg->socket_main->address[i]->text();
                if(i<3)
                    this->sys_flag->ip_main+=".";
            }
            this->sys_flag->port_main=quint16(this->alg->socket_main->port->text().toInt());
            //qDebug()<<this->sys_flag->ip_main<<this->sys_flag->port_main;
        }
    }
}
...
void algorithm_file::socket_disconnect()
{
    ...
    else
    {
        ...
        if(start_run==false)
        {
            this->alg->main_connect->setText("connect");
        }
    }
...
void algorithm_file::relation_init()
{
    for(int i=0;i<this->simulation_group->group.size();i++)
    {
        Simulation_Moudle* moudle=this->simulation_group->group[i];
...
void algorithm_file::detect_loop()
{
    for(int i=0;i<this->simulation_group->group.size();i++)
    {
...
void algorithm_file::loop_check(Simulation_Moudle* moudle,int index)
{
    if(moudle->loop_flag)
    {
